...
public class TrybeGamesDatabase
{
    ...
    public List<GameWithStudio> GetGamesWithStudio()
    {
        ...
        {
            GameName = game.Name,
            StudioName = GameStudios.FirstOrDefault(studio => studio.Id == game.DeveloperStudio)?.Name,
    ...
    public List<StudioGamesPlayers> GetStudiosWithGamesAndPlayers()
    {
        return GameStudios.Select(studio => new StudioGamesPlayers
        {
            ...
            {
                GameName = game.Name,
                Players = Players.Where(player => player.FavoriteGameStudios.Contains(studio.Id)).ToList()
            }).ToList()
        }).ToList();
    }
